...
export function UsageMeters({
  ...
  const rows = [
    { label: t('usage.agents'), value: agentCount, max: limits.maxAgents },
    { label: t('usage.runs'), value: usage.agentRuns, max: limits.maxAgentRuns },
    {
      label: t('usage.tokens'),
      value: usage.tokensApprox,
      max: limits.maxTokensApprox,
    },
    { label: t('usage.needsYou'), value: usage.escalations, max: null as number | null },
  ];
  const softHit =
  ...
  return (
    <div className="space-y-3">
      ...
      <div
        className={cn('grid gap-3', compact ? 'grid-cols-2 md:grid-cols-4' : 'sm:grid-cols-2')}
      >
        {rows.map((row) => {
          const pct =
            row.max && row.max > 0
                ...
                <div className="h-1.5 rounded-full bg-border overflow-hidden">
                  <div
                    className={cn(
                      ...
                      hot ? 'bg-urgent' : 'bg-foreground/70'
                    )}
                    style={{ width: `${pct}%` }}
                  />
                </div>
