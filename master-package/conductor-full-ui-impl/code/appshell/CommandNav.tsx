 ...
 * Integrate into existing AppShell.tsx.
 * Pass pendingEscalations from existing hooks.
 */
...
const mainNav = [
  ...
  { href: '/escalations', label: 'Needs You', icon: AlertTriangle, badge: true },
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex fixed inset-y-0 left-0 z-40 w-[68px] flex-col items-center border-r border-border bg-background/95 py-4 gap-1">
        <div className="mb-6 text-[11px] font-semibold tracking-widest text-muted-foreground">
          C
        </div>
        {mainNav.map((item) => {
          const active = pathname.startsWith(item.href);
          const Icon = item.icon;
          const showBadge = item.badge && pendingEscalations > 0;
          return (
            <Link
              key={item.href}
              href={item.href}
      ...
      {/* Mobile bottom tabs */}
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-40 border-t border-border bg-background/95 backdrop-blur supports-[padding:max(0px)]:pb-[env(safe-area-inset-bottom)]">
          ...
          {mainNav.map((item) => {
            ...
            return (
              ...
              >
                ...
                {showBadge && (
                  <span className="absolute right-[22%] top-0.5 h-1.5 w-1.5 rounded-full bg-red-600" />
                )}
              </Link>
            );
          })}
